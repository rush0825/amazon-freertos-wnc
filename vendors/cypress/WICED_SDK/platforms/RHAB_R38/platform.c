...
const platform_gpio_t platform_gpio_pins[] =
{
    ...
    [WICED_GPIO_28] = { GPIOE, 12 }, // Sharn add
    [WICED_GPIO_29] = { GPIOC, 6 }, // Sharn add
    [WICED_GPIO_30] = { GPIOC, 7 }, // Sharn add
    [WICED_GPIO_31] = { GPIOB, 1 }, // David add
    [WICED_GPIO_32] = { GPIOB, 12 }, // David add for LED3
    [WICED_GPIO_33] = { GPIOB, 13 }, // David add for LED4
    [WICED_GPIO_34] = { GPIOB, 5 }, // David add for SW2 Button
};
...
const platform_i2c_t platform_i2c_peripherals[] =
{
    ...
    {
        .port                    = I2C1,
        .pin_scl                 = &platform_gpio_pins[WICED_GPIO_24],  // Sharn EVB J12 pin 10 , connect to MFI pin 1
        .pin_sda                 = &platform_gpio_pins[WICED_GPIO_25],  // Sharn EVB J12 pin 8  , connect to MFI pin 3
...
/*        Sharn For MFI chip,           *
 * J12 pin6(GND), connect to MFI pin 2  *
 * JP9 pin3(VDD), connect to MFI pin 5  */
...
const platform_mfi_auth_chip_t platform_auth_chip =
{
    ...
    .reset_pin  = WICED_GPIO_28,  // Sharn EVB J12 pin 14 , connect to MFI pin 8
};
...
const platform_uart_t platform_uart_peripherals[] =
{
    ...
    /* Sharn add WICED_UART_3*/
    [WICED_UART_3] =
       {
           .port               = USART6,
           .tx_pin             = &platform_gpio_pins[WICED_GPIO_29],
...
// PB12: PB13 are confilt with LED... and can not found wwd pin spi_IRQ
// Sharn say that do not care this...
...
void platform_init_peripheral_irq_priorities( void )
{
    ...
    NVIC_SetPriority( USART6_IRQn      ,  6 ); /* WICED_UART_3        */   // Sharn add
    ...
    NVIC_SetPriority( DMA2_Stream6_IRQn,  7 ); /* WICED_UART_3 TX DMA */   // Sharn add
    NVIC_SetPriority( DMA2_Stream1_IRQn,  7 ); /* WICED_UART_3 RX DMA */   // Sharn add
...
{
    /* Init for Alltek smart meter */
    /*
     * RELAY_BUTTON1                   SWI_1
     * RELAY_CONTROL1                  SWO_1
     * RELAY_BUTTON2                   SWI_2
     * RELAY_CONTROL2                  SWO_2
     */
#if 0 //WICED_GPIO_13~WICED_GPIO_17 are JTAG pin, so disable
    platform_gpio_init( &platform_gpio_pins[WICED_GPIO_13], INPUT_PULL_UP );        // AP_MODE_SWI
    platform_gpio_init( &platform_gpio_pins[WICED_GPIO_18], OUTPUT_PUSH_PULL );     // WIFI_LED
    platform_gpio_init( &platform_gpio_pins[WICED_GPIO_14], OUTPUT_PUSH_PULL );     // SWO_1
    platform_gpio_init( &platform_gpio_pins[WICED_GPIO_15], OUTPUT_PUSH_PULL );     // SWO_2
    platform_gpio_init( &platform_gpio_pins[WICED_GPIO_16], INPUT_PULL_UP );        // SWI_1
    platform_gpio_init( &platform_gpio_pins[WICED_GPIO_17], INPUT_PULL_UP );        // SWI_2
    //platform_gpio_init( &platform_gpio_pins[WICED_GPIO_10], OUTPUT_PUSH_PULL );     // MFI_RST

    platform_gpio_output_low( &platform_gpio_pins[WICED_GPIO_15] );     // relay2 set off
    platform_gpio_output_low( &platform_gpio_pins[WICED_GPIO_14] );     // relay1 set off
    platform_gpio_output_high( &platform_gpio_pins[WICED_GPIO_18] );    // WIFI_LED off
    //platform_gpio_output_low( &platform_gpio_pins[WICED_GPIO_10] );     // MFI_RST off
    //
#endif
    ...
    platform_gpio_init( &platform_gpio_pins[WICED_BUTTON3], INPUT_PULL_UP );
    platform_gpio_init( &platform_gpio_pins[WICED_BUTTON4], INPUT_PULL_UP );
...
void platform_pull_WP( void )
{
    platform_gpio_init( &platform_gpio_pins[WICED_GPIO_31], OUTPUT_PUSH_PULL ); // SPI Flash PB1 Pull High
}
